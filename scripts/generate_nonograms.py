...
class NonogramInstance:
    ...
    def hint_to_dfa(self, hint: NDArray[np.int_]) -> str:
        ...
        return json.dumps({'Q': q, 'S': 2, 'd': d.tolist(), 'q0': 1, 'F': [q]})

    def hint_to_cdfa(self, hint: list[int]) -> str:
        # TODO 
        return ""

    def minizinc(self, seed: int) -> str:
        # Start by describing the instance
        output: list[str] = [
            "%",
            "% Instance for Nonogram model,",
            "% generated using experiments/problem_generators/nonogram.py",
            "%",
        ...
        for row in self.grid:
            output.append("%   " + " ".join([str(cell + 1) for cell in row]))
...
def main():
    ...
    # print([nonogram.column_hint(x) for x in range(args.width)])
    # print([nonogram.row_hint(y) for y in range(args.height)])
    print(nonogram.minizinc(args.seed))
    data_file = os.path.join(
        os.path.realpath(args.data),
        "nonogram_"
        f"{args.width}x{args.height}_"
        f"seed_{args.seed}.dzn"
    )
    with open(data_file, "w") as data_file:
        print(nonogram.minizinc(args.seed), file=data_file)
